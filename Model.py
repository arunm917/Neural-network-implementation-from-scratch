...
class Model:
    ...
    def fit(
        ...
        opt = Optimizer(
            self.W, self.B, self.sizes, batch_size, learning_rate, optimizer
        )
        ...
        for i in trange(epochs, total=epochs, unit="epoch"):
            ...
            for i in range(0, len(X), batch_size):
                ...
                (self.dW, self.dB) = self.grad(
                    self.x, self.y, activation_function, W=None, B=None
                )
...
def fit(
    ...
    for i in trange(epochs, total=epochs, unit="epoch"):
        ...
        accuracy_val = accuracy_score(Y_pred_val, Y_val)
        print("validation accuracy = ", round(accuracy_val, 2))
        # wandb.log({'loss_epoch': loss_epoch, 'accuracy_val':accuracy_val})
